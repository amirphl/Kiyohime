import React from 'react';
import { MessageSquare } from 'lucide-react';
import Card from '../../ui/Card';
import { useLanguage } from '../../../hooks/useLanguage';

interface MessageCountCardProps {
    messageCount?: number;
    maxMessageCount?: number;
    isLoading: boolean;
    error: string | null;
    lastApiCall: number;
    lineNumber: string;
    totalBudget: number;
    title: string;
    calculatingLabel: string;
    messagesLabel: string;
    messageCountHelpText: string;
    calculatingText: string;
    enterBudgetText: string;
    lastUpdatedLabel: string;
}

const MessageCountCard: React.FC<MessageCountCardProps> = ({
    messageCount,
    maxMessageCount,
    isLoading,
    error,
    lastApiCall,
    lineNumber,
    totalBudget,
    title,
    calculatingLabel,
    messagesLabel,
    messageCountHelpText,
    calculatingText,
    enterBudgetText,
    lastUpdatedLabel,
}) => {
    const { language } = useLanguage();

    const formatLastUpdated = (timestamp: number): string => {
        if (!timestamp || timestamp <= 0) return '';
        const date = new Date(timestamp);
        if (language === 'fa') {
            try {
                const formatter = new Intl.DateTimeFormat('fa-IR-u-ca-persian', {
                    dateStyle: 'medium',
                    timeStyle: 'short',
                    hour12: false,
                    timeZone: 'UTC',
                });
                return formatter.format(date);
            } catch {
                return date.toUTCString();
            }
        }
        return date.toLocaleString();
    };

    const renderContent = () => {
        if (isLoading) {
            return (
                <div className="flex items-center space-x-2 text-gray-600">
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-primary-600"></div>
                    <span>{calculatingLabel}</span>
                </div>
            );
        }

        if (messageCount !== undefined) {
            return (
                <div className="space-y-2">
                    <div className="text-2xl font-bold text-primary-600">
                        {messageCount.toLocaleString()}
                        {typeof maxMessageCount === 'number' ? ` / ${maxMessageCount.toLocaleString()}` : ''}
                        {' '}{messagesLabel}
                    </div>
                    {lastApiCall > 0 && (
                        <div className="text-xs text-gray-500">
                            {lastUpdatedLabel.replace('{time}', formatLastUpdated(lastApiCall))}
                        </div>
                    )}
                </div>
            );
        }

        if (error) {
            return <div className="text-red-600 text-center">{error}</div>;
        }

        return (
            <div className="text-gray-500 text-center">
                {lineNumber && totalBudget > 0 ? calculatingText : enterBudgetText}
            </div>
        );
    };

    return (
        <Card>
            <div className="space-y-4">
                <h3 className="text-lg font-medium text-gray-900 flex items-center">
                    <MessageSquare className="h-5 w-5 mr-2 text-primary-600" />
                    {title}
                </h3>

                <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                    {renderContent()}
                </div>
            </div>
        </Card>
    );
};

export default MessageCountCard; 