...
export const buildProformaHtml = (raw: ProformaData, language: string) => {
  ...
  const locale = (language || data?.lang || 'en').toLowerCase() === 'fa'
    ? 'fa'
    : 'en';
  ...
  const sellerNationalId = pick(seller.national_id, DEFAULT_SELLER.national_id);
  const sellerRegistrationNumber = pick(
    seller.registration_number,
    DEFAULT_SELLER.registration_number
  );
  const sellerPostalCode = pick(seller.postal_code, DEFAULT_SELLER.postal_code);
  ...
  const sellerIban = pick(seller.iban, seller.sheba, DEFAULT_SELLER.iban);
  ...
  const footerNotes = data?.notes ? `<div class="notes">${escapeHtml(data.notes)}</div>` : '';
