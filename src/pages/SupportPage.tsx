...
const SupportPage: React.FC = () => {
  ...
  const fetchTickets = async (force: boolean = false) => {
    ...
    try {
      const resp = await apiService.listSupportTickets({
        page: 1,
        page_size: 50,
      });
  ...
  const validate = (): boolean => {
    ...
    if (!content.trim())
      errs.content = t('dashboard.supportModal.validation.descriptionRequired');
    else if (content.length > 1000)
      errs.content = t('dashboard.supportModal.validation.descriptionMax');
    if (file) {
      const allowed = [
        'image/jpeg',
        'image/png',
  ...
  const handleDownloadAttachment = async (
    ...
    try {
      const resp = await apiService.downloadTicketAttachment(
        ticketId,
        fileIndex
      );
      if (resp.success && resp.data?.blob) {
        ...
        const msg =
          resp.message ||
          t('dashboard.supportModal.downloadError') ||
      ...
      const msg = isTimeout
        ? t('dashboard.supportModal.downloadTimeout') ||
  ...
  const handleCreateTicket = async () => {
    ...
    try {
      ...
      if (resp.success) {
        ...
        resetForm();
        alert(
          t('dashboard.supportModal.success') || 'Ticket created successfully'
        );
  ...
  const validateReply = (): boolean => {
    if (!replyContent.trim()) {
      setReplyError(
        t('dashboard.supportModal.validation.descriptionRequired') ||
          'Content is required'
      );
      return false;
    }
    if (replyContent.length > 1000) {
      setReplyError(
        t('dashboard.supportModal.validation.descriptionMax') ||
    ...
    if (replyFile) {
      ...
      if (!allowed.includes(replyFile.type)) {
        setReplyError(
          t('dashboard.supportModal.validation.invalidType') ||
            'Invalid file type'
        );
        return false;
      }
      if (replyFile.size > 10 * 1024 * 1024) {
        setReplyError(
          t('dashboard.supportModal.validation.maxSize') || 'File is too large'
        );
        return false;
      }
    }
  ...
  const handleReply = async () => {
    ...
    try {
      const resp = await apiService.createTicketReply({
        ticket_id: ticketId,
        ...
        <Card>
          <div className='space-y-4'>
            <p className='text-gray-700'>
              {t('dashboard.supportIntro') ||
                'Open a ticket to contact our support team.'}
            </p>
            <div className='mt-6'>
              <h2 className='text-lg font-medium text-gray-900 mb-3'>
                {t('dashboard.supportHistory') || 'Tickets History'}
              </h2>
              {ticketsError && (
                    ...
                    <thead className='bg-gray-50'>
                      <tr>
                        <th className='px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
                          {walletCopy.table.row || '#'}
                        </th>
                        <th className='px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
                          {t('dashboard.supportModal.titleLabel') || 'Title'}
                        </th>
                        <th className='px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
                          {t('dashboard.supportModal.contentLabel') ||
                            'Content'}
                        </th>
                        <th className='px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider'>
                          {walletCopy.table.datetime || 'Created At'}
                        </th>
                    ...
                    <tbody className='bg-white divide-y divide-gray-200'>
                      {groups.length === 0 ? (
                        <tr>
                          ...
                          >
                            {walletCopy.table.noTransactions || 'No records'}
                          </td>
                        </tr>
                      ) : (
                        groups.map((g, idx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td
                                className='px-4 py-3 whitespace-nowrap text-sm text-gray-900 max-w-[360px] truncate'
                                title={last?.title || ''}
                              >
                                {last?.title || '-'}
                              </td>
                              <td
                                className='px-4 py-3 text-sm text-gray-500 max-w-[600px] truncate'
                                title={last?.content || ''}
                              >
                                {last?.content || '-'}
                              </td>
      ...
      >
        <div className='space-y-4'>
          <div>
            ...
            >
              {t('dashboard.supportModal.titleLabel') || 'Title (optional)'}
            </label>
            <input
              id='ticketTitle'
              type='text'
              ...
              placeholder={
                t('dashboard.supportModal.titlePlaceholder') ||
                'Enter a short title (<= 80 chars)'
              }
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={
                t('dashboard.supportModal.contentPlaceholder') ||
                'Describe your request (<= 1000 chars)'
              }
              className='w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 border-gray-300'
            />
            {errors?.content && (
          ...
          {selectedGroup ? (
            <>
              <div className='text-xs uppercase tracking-[0.2em] text-gray-400'>
                {t('dashboard.supportModal.ticketSectionTitle') ||
                  'Ticket Details'}
              </div>
              <div className='space-y-4'>
                {selectedGroup.items.map(it => {
                  ...
                  return (
                    <div
                      key={it.id}
                      className='p-4 border border-gray-200 rounded-lg bg-gray-50/60 relative'
                    >
                      {it.replied_by_admin && (
                        <div className='absolute top-3 right-3'>
                          <span className='inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800'>
                            {t('dashboard.supportModal.adminReply') ||
                              'Admin Reply'}
                          </span>
                        </div>
                      )}
                      <div className='text-xs uppercase tracking-wide text-gray-500'>
                        {t('dashboard.supportModal.metadataTitle') ||
                          'Metadata'}
                      </div>
                      <div className='mt-2 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm'>
                        <div>
                          <div className='text-xs text-gray-500'>
                            {t('dashboard.supportModal.createdAtLabel') ||
                              'Created at'}
                          </div>
                      ...
                      <div className='mt-4 border-t border-gray-200 pt-4'>
                        <div className='text-xs uppercase tracking-wide text-gray-500'>
                          {t('dashboard.supportModal.messageTitle') ||
                            'Message'}
                        </div>
                      ...
                      <div className='mt-4 border-t border-gray-200 pt-4'>
                        <div className='text-xs uppercase tracking-wide text-gray-500'>
                          {t('dashboard.supportModal.attachments') ||
                            'Attachments'}
                        </div>
                        {attachments.length > 0 ? (
                            ...
                            {attachments.map((attachment, idx) => {
                              const label =
                                typeof attachment === 'string'
                                  ...
                                  : attachment?.filename ||
                                    attachment?.name ||
                                    `${t('dashboard.supportModal.fields.attachment') || 'Attachment'} ${idx + 1}`;
                              return (
                                <button
                                  key={`${it.id}-${idx}`}
                                  type='button'
                                  onClick={() =>
                                    handleDownloadAttachment(it.id, idx, label)
                                  }
                                  className='text-left text-sm text-primary-600 hover:underline'
                                >
                                  {t(
                                    'dashboard.supportModal.downloadAttachment'
                                  ) || 'Download attachment'}{' '}
                                  {label ? `(${label})` : ''}
                                </button>
                              );
                          ...
                          <div className='mt-2 text-sm text-gray-500'>
                            {t('dashboard.supportModal.noAttachments') ||
                              'No attachments'}
                          </div>
                        )}
                      </div>
        ...
        <div className='space-y-4'>
          <div>
            ...
            <textarea
              ...
              placeholder={
                t('dashboard.supportModal.contentPlaceholder') ||
                'Describe your reply (<= 1000 chars)'
              }
              className='w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 border-gray-300'
            />
            {replyError && (
