          ...
          <thead className='bg-gray-50'>
            <tr>
              ...
              <th className='px-4 py-2 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide'>
                {copy.discountsTable.rate}
              </th>
              <th className='px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide'>
                {copy.discountsTable.created}
              </th>
              <th className='px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide'>
                {copy.discountsTable.expires}
              </th>
              <th className='px-4 py-2 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide'>
                {copy.discountsTable.totalSent}
              </th>
              <th className='px-4 py-2 text-right text-xs font-semibold text-gray-500 uppercase tracking-wide'>
                {copy.discountsTable.agencyIncome}
              </th>
            </tr>
          </thead>
              ...
              items.map((d, idx) => (
                <tr key={`${d.created_at}-${idx}`} className='hover:bg-gray-50'>
                  ...
                  <td className='px-4 py-2 text-right'>
                    {typeof d.discount_rate === 'number'
                      ? `${(d.discount_rate * 100).toFixed(2)}%`
                      : '-'}
                  </td>
                  <td className='px-4 py-2'>{formatDateTime(d.created_at)}</td>
                  <td className='px-4 py-2'>{formatDateTime(d.expires_at)}</td>
                  <td className='px-4 py-2 text-right'>
                    {formatNumber(d.total_sent)}
                  </td>
                  <td className='px-4 py-2 text-right'>
                    {formatNumber(d.agency_share_with_tax)}
                  </td>
                </tr>
              ))
            ) : (
