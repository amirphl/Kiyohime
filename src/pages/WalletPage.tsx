...
const WalletPage: React.FC = () => {
  ...
  const [balanceLastUpdated, setBalanceLastUpdated] = useState<string | null>(
    null
  );
  ...
  const recommendedAmounts = [
    5000000, 10000000, 15000000, 30000000, 50000000, 100000000,
  ];
            ...
            <div className='space-y-2'>
              ...
              <div className='grid grid-cols-4 gap-4'>
                <div className='bg-gray-50 p-4 rounded-lg'>
                  ...
                  <p className='text-xl font-semibold'>
                    {loadingBalance && balanceFree === null
                      ? t('common.loading')
                      : `${(balanceFree ?? 0).toLocaleString()} ${currencyLabel}`}
                  </p>
                </div>
                <div className='bg-gray-50 p-4 rounded-lg'>
                  <p className='text-sm text-gray-500'>{t('wallet.locked')}</p>
                  <p className='text-xl font-semibold'>
                    {loadingBalance && balanceLocked === null
                      ? t('common.loading')
                      : `${(balanceLocked ?? 0).toLocaleString()} ${currencyLabel}`}
                  </p>
                </div>
                <div className='bg-gray-50 p-4 rounded-lg'>
                  ...
                  <p className='text-xl font-semibold'>
                    {loadingBalance && balanceReserved === null
                      ? t('common.loading')
                      : `${(balanceReserved ?? 0).toLocaleString()} ${currencyLabel}`}
                  </p>
                </div>
                <div className='bg-gray-50 p-4 rounded-lg'>
                  <p className='text-sm text-gray-500'>
                  ...
                  <p className='text-xl font-semibold'>
                    {loadingBalance && balanceCredit === null
                      ? t('common.loading')
                      : `${(balanceCredit ?? 0).toLocaleString()} ${currencyLabel}`}
                  </p>
                </div>
              </div>
              <div className='grid grid-cols-2 gap-4'>
                <div className='bg-gray-50 p-4 rounded-lg'>
                  ...
                  <p className='text-xl font-semibold'>
                    {loadingBalance && balanceTotal === null
                      ? t('common.loading')
                      : `${(balanceTotal ?? 0).toLocaleString()} ${currencyLabel}`}
                  </p>
                </div>
                <div className='bg-gray-50 p-4 rounded-lg'>
                  <p className='text-sm text-gray-500'>
                    {t('wallet.lastUpdated') || 'Last Updated'}
                  </p>
                  <p className='text-sm font-medium text-gray-700'>
                    {balanceLastUpdated
                      ? formatDatetime(balanceLastUpdated)
                      : '-'}
                  </p>
                </div>
              </div>
                ...
                <table className='min-w-full divide-y divide-gray-200'>
                  <thead className='bg-gray-50'>
                    <tr>
                      ...
                      <th className='px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider'>
                        {t('wallet.table.freeIncrease')}
                      </th>
                      <th className='px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider'>
                        {t('wallet.table.creditIncrease')}
                      </th>
                      <th className='px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider'>
                        {t('wallet.table.description')}
                      </th>
                    </tr>
                  </thead>
                  <tbody className='bg-white divide-y divide-gray-200'>
                    ...
                    {historyItems.map((item, idx) => {
                      const freeBefore =
                        (item.balance_before &&
                          (item.balance_before as any)['free']) ??
                        0;
                      const freeAfter =
                        (item.balance_after &&
                          (item.balance_after as any)['free']) ??
                        0;
                      const creditBefore =
                        (item.balance_before &&
                          (item.balance_before as any)['credit']) ??
                        0;
                      const creditAfter =
                        (item.balance_after &&
                          (item.balance_after as any)['credit']) ??
                        0;
                      ...
                      return (
                        <tr key={item.uuid} className='hover:bg-gray-50'>
                          ...
                          <td className='px-4 py-3 whitespace-nowrap text-sm text-gray-900 text-center'>
                            {t(`wallet.operationTypes.${item.operation}`)}
                          </td>
                          <td className='px-4 py-3 whitespace-nowrap text-sm text-gray-900 text-center'>
        ...
        <div className='space-y-3'>
          <div className='flex justify-between'>
            <span className='text-gray-600'>
              {t('wallet.modalChargeAmount')}
            </span>
            <span className='font-medium'>
              {Number(chargeAmount).toLocaleString()} {currencyLabel}
            </span>
          </div>
          <div className='flex justify-between'>
            <span className='text-gray-600'>{t('wallet.modalTax')}</span>
            <span className='font-medium'>
              {Math.round(Number(chargeAmount) * 0.1).toLocaleString()}{' '}
              {currencyLabel}
            </span>
          </div>
          <div className='flex justify-between border-t border-gray-200 pt-2'>
            ...
            <span className='font-bold'>
              {(
                Number(chargeAmount) + Math.round(Number(chargeAmount) * 0.1)
              ).toLocaleString()}{' '}
              {currencyLabel}
            </span>
          </div>
